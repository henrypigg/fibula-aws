    ...
    return (
        <div>
            <center>
                ...
                <div>
                    <Button variant={category === 'Pending' ? 'contained' : 'text'} color="primary" onClick={() => setCategory('Pending')}>Pending</Button>
                    <Button variant={category === 'Approved' ? 'contained' : 'text'}  color="primary" onClick={() => setCategory('Approved')}>Approved</Button>
                    <Button variant={category === 'Denied' ? 'contained' : 'text'}  color="secondary" onClick={() => setCategory('Denied')}>Denied</Button>
                </div>
                <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    ...
                    <TableRow>
                        ...
                        {category !== 'Approved' ? <TableCell align="right">Approve</TableCell> : null}
                        {category !== 'Denied' ? <TableCell align="right">Deny</TableCell> : null}
                    </TableRow>
                    </TableHead>
                    <TableBody>
                    {data.filter((request) => (request.enrollmentstatus === category)).map((row) => (
                        <ExpandableTableRow
                        email={row.email}
                        key={row.requestid}
                        ...
                        >
                            ...
                            {category !== 'Approved' ? <TableCell align="right"><Button variant="contained" color="primary" onClick={() => handleApprove(row)}>Approve</Button></TableCell> : null}
                            {category !== 'Denied' ? <TableCell align="right"><Button variant="contained" color="secondary" onClick={() => handleDeny(row)}>Deny</Button></TableCell> : null}
                        </ExpandableTableRow>
                    ))}
                    </TableBody>
                </Table>
